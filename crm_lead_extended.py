...
#        res = super(res_partner, self).name_get(cr, uid, ids,context=context)
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.last_name:
                ...
                last_name = last_name.replace('\n\n','\n')
            print '\n name get of partner name and last name',record.name,record.last_name
            res1.append((record.id, '%s %s' %(name, last_name)))
        print '\n name get of partner res',res1
        return res1
    ...
    _columns = {
        ...
        'partner_full_name' : fields.function(_combine, string="Name", type='char', method=True, arg=('name', 'last_name'), store={
                'res.partner': (lambda self, cr, uid, ids, c={}: ids, ['name', 'last_name'], 10),
            },),
...
class res_dealer(osv.Model):
    ...
    def _appointment_trips_count(self, cr, uid, ids, field_name, arg, context=None):
        res = dict(map(lambda x: (x,{'appointment_count': 0, 'trips_count': 0, 'demos_count' : 0, 'contest_count' : 0, 'sales_count' : 0, 'total_income' : 0, 'ratio_count' : 0.0,}), ids))
        # the user may not have access rights for opportunities or meetings
        try:
            for dealer in self.browse(cr, uid, ids, context):
                ...
                ratio_count = len(demo_ids) / len(sale_ids)
                res[dealer.id] = {
...
#                            'return_date' : stock_pick_objs[0].date_done,
    ...
    _columns = {
        ...
        'last_name': fields.char('Last Name', required=True),
        'dealer_full_name' : fields.function(_combine, string="Name", type='char', method=True, arg=('name', 'last_name'), store={
                'res.dealer': (lambda self, cr, uid, ids, c={}: ids, ['name', 'last_name'], 10),
            },),
        ...
        'home_phone_code' : fields.char('Code'),
        'home_phone': fields.char('Home Phone'),
        'home_phone_code2' : fields.char('Code'),
        'home_phone2': fields.char('Home Phone (2)'),
        'work_phone_code' : fields.char('Code'),
        'work_phone': fields.char('Work Phone'),
        'dealer_email': fields.char('Email'),
        ...
        'part_full_time': fields.selection([('full time', 'Full Time'), ('part time', 'Part Time')], 'Status'),
        ...
        'ratio_count' : fields.function(_appointment_trips_count, type="float", string='Ratio', multi="appointment_count"),
        'default_add_1' : fields.boolean('Default'),
        'default_add_2' : fields.boolean('Default'),
        'defautl_comm_level' : fields.char('Default Commision Level'),
...
class calendar_event(osv.Model):
    ...
    def _combine(self, cr, uid, ids, field_name, args, context=None):
        ...
        for id in ids:
            rec = self.browse(cr, uid, [id], context=context)[0]
            values[id] = {}
            values[id] = '%s %s' % (rec.first_name, rec.last_name)
        return values
        
    def onchange_stage(self, cr, uid, ids, stage_id, context=None):
        print '\n#$%^%$#$%^%## onchange_stage',ids,stage_id,context
        res = {}
        if stage_id:
            res['value'] = {'stage_id_2' : stage_id}
        return res
        
#    def _distance_mesurement(self, cr, uid, ids, field_name, arg, context=None):
#        res = dict(map(lambda x: (x,{'distance': 0}), ids))
#        Company_pool = self.pool.get('res.company')
#        try:
#            for appointment in self.browse(cr, uid, ids, context):
#               src_street = appointment.street
#               src_street2 = appointment.street2
#               src_city = appointment.city
#               src_state = appointment.state_id
#               src_zip = appointment.zip
#               src_country = appointment.country_id
#               
#               company = Company_pool.search(cr, uid, [('')])
 #       except:
#            pass
#        return res
    ...
    _columns = {
            ...
            'event_full_name' : fields.function(_combine, string="Name", type='char', method=True, arg=('first_name', 'last_name'), store={
                'calendar.event': (lambda self, cr, uid, ids, c={}: ids, ['first_name', 'last_name'], 10),
            },),
            'street': fields.char('Street'),
            'street2': fields.char('Street2'),
            ...
            'user_id': fields.many2one('res.users', 'Team Leader', states={'done': [('readonly', True)]}),
            'partner_id' : fields.many2one('res.partner', 'Partner'),
            ...
            'ride_along_id_1' : fields.many2one('res.lead', 'Ride-Along'),
            'phone_code' : fields.char('Code'),
            ...
            'assistant_id_2' : fields.many2one('res.dealer', 'Assistant'),
            ...
            'partner_phone_code' : fields.char('Code'),
            'partner_phone' : fields.char('Mobile Phone'),
            ...
            'child_1' : fields.char('Child 1'),
            'child_2' : fields.char('Child 2'),
            'stage_id_2': fields.selection([('new', 'New'), ('appoint_set', 'Appointment Set'), ('appoint_cancel', 'Appointment Cancelled'), ('demo_in_prog', 'Demo in Progress')], 'Status'),
            
#            'distance' : fields.function(_distance_mesurement, type="char", string='Distance'),
    }
    ...
    _defaults = {
        'name': lambda self,cr,uid,context={}: self.pool.get('ir.sequence').get(cr, uid, 'calendar.event') + ' / ' + datetime.strptime(str(date.today()), "%Y-%m-%d").strftime("%B, %Y") ,
        'stage_id' : 'new',
    }
...
class crm_phonecall(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        print '\n create of phonce call ',vals,context
        if vals.get('opportunity_id'):
            next_action_date = datetime.strptime(vals.get('date'), "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d")
            print '\n create of phonce call ',vals.get('opportunity_id')
            Lead_pool.write(cr, uid, vals.get('opportunity_id'), {'next_action' : next_action_date})
...
class crm_campaign(osv.osv):
    ...
    def onchange_product(self, cr, uid, ids, gift_id, context=None):
        print '\n onchange of onchange_product',ids,context,gift_id
        result = {}
        partner_obj = self.browse(cr, uid, ids)
        if gift_id:
            produ_rec = self.pool.get('product.product').browse(cr, uid, gift_id)
            result['value'] = {'image' : produ_rec.image_medium}
        return result
        
    _columns = {
    
            'name' : fields.char('Name'),
            'desc' : fields.char('Campaign Description'),
            'active' : fields.boolean('Active'),
            'gift_id' : fields.many2one('product.product', 'Gift', domain="[('prod_gift', '=', True)]"),
            'available_for' : fields.selection([('owner', 'Owner'), ('non owner', 'Non Owner')], 'Available for'),
            'start_date' : fields.date('Start Date'),
            'end_date' : fields.date('End Date'),
            'image' : fields.binary('Image'),
            
            'required_demos' : fields.char('Required Demos'),
            'sale_required' : fields.char('Required Sales'),
            'complete_within_day' : fields.char('Allowed Days'),
            ...
            'team_bool' : fields.boolean('Team Bool'),
            'company_bool' : fields.boolean('Company Bool'),
            'sales_team_ids': fields.many2many('crm.case.section', 'campaign_sales_team_rel', 'crm_campaign', 'case_section', string='Available for Sales Team'),
            'company_ids': fields.many2many('res.company', 'campaign_company_rel', 'crm_campaign', 'res_comp', string='Available for Companies'),
...
class crm_lead2appointment(osv.osv_memory):
    ...
    def onchange_start_date(self, cr, uid, ids, start_datetime, context=None):
        result = {}
        if start_datetime:
            start_d_time = datetime.strptime(start_datetime, "%Y-%m-%d %H:%M:%S")
            stop_datetime = start_d_time + timedelta(minutes=90)
            result['value'] = {'stop_datetime' : stop_datetime}
        return result
    ...
    def action_apply(self, cr, uid, ids, context=None):
        ...
        next_action_date = datetime.strptime(wizard.start_datetime, "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d")
        print '\n action apply####',next_action_date
        vals = {'first_name' : crm_lead_obj.name,
                'last_name' : crm_lead_obj.last_name,
                'phone' : crm_lead_obj.personal_no,
                'dealer_id' : crm_lead_obj.dealer,
                'lead_type' : crm_lead_obj.lead_type,
                'section_id' : crm_lead_obj.section_id,
                'start_datetime' : wizard.start_datetime,
                'stop_datetime' : wizard.stop_datetime,
                'stage_id' :'new',
                'image' : crm_lead_obj.image,
                'lead_id' : crm_lead_obj.id,}
        ...
        Lead_obj.write(cr, uid, context.get('active_id'), {'stage_id': stage_id, 'next_action' : next_action_date})
